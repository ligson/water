...
源码采用前后端分离，但保持部署思路简单。生产构建时前端 `dist` 通过 Go `embed` 编译进同一个 `water` 二进制；开发时仍可分别运行 Go 后端和 Vite，便于快速热更新。
...
- Base URL。
- Model。
- Model 字段支持手动填写，也支持通过 OpenAI-compatible 标准 `/models` 接口拉取候选模型后选择；如果本地模型网关没有实现该接口，页面仍允许手动填写。
...
后端服务器终端必须使用独立 WebSocket 通道，不混入任务事件流。

- 终端主要服务于人工登录 Water 后端所在服务器、手动排障和交互式开发。
- 终端会话按工作区绑定，默认进入当前工作区根目录。
- 第一版不做多连接配置，也不要求用户配置主机、端口、密码或私钥。
- 终端输出不写入 `turn` / `tool` / `approval` 事件流，避免把手动 shell 与 Harness 执行搅在一起。
- 如果未来需要让 Agent 触达终端，应单独设计受控执行器，不复用人工终端会话。
- 前端已经把终端放进右侧栏独立 `Terminal` Tab，并使用 `xterm.js` 渲染交互式 shell；聊天区不承载终端流量。
